...
install.packages(c("interp", "oce", "Matrix", "knitr"))
#library.dynam(interp)
library(oce)
library(Matrix)
library(knitr)
...
```{r}
knitr::kable(SEA_MICRO[1:16,], format = "pipe", caption = "Entries 1-16 of the SEA_MICRO Dataset")
```
...
incr.by <- function(s, e, by) {
    ...
    if (length(seps) == 1) {
      ...
      #debug point! print("you gave me a range of one, so here's it back.");
      return(seps)
    } 
    #incr as appropos
    negs.set <- seps[[1]];
    ...
    set.all <- c(negs.set, zeds.set, overs.set);
    return(set.all)
}
...
get.set.gridvals <- function(this.set) {
    this.r.long <- range2floorceil(this.set$longitude);
    v <- this.r.long;
    if (length(v)==1) {
        if (is.null(v)) {
            return(FALSE)
    ...
    this.r.lat <- range2floorceil(this.set$latitude);
    v <- this.r.lat;
    if (length(v)==1) {
        if (is.null(v)) {
            return(FALSE)
...
xg <- ws.1986.gv[1]
yg <- ws.1986.gv[2]
if (is.logical(xg) || is.null(xg)) {
  stop("something went wrong with get.set.gridvals")
} else if (is.logical(yg) || is.null(yg)) {
  stop("something went wrong with get.set.gridvals")
}
...
get.actual <- function(year.index) {
    ...
    #debug point! print("get.actual: next is gridvals");
    gv <- get.set.gridvals(this.set);
    if (is.logical(gv)) {
        return(FALSE)
      }
    #debug point! print("get.actual: gridvals done, next is ib");
...
### RSS COMPUTATION AND RESULTS

After a considerable amount of runtime passes, the code we have written for this paper computed the following values for the RSS value of each model at each successive year. This code is far from optimal, and additional work could be done to improve its functionality, complexity, and speed. As you can see from the table below, the values are incredibly large; once caveat is that no observations were taken in 1988, and, as we could not determine a value for that model because of this absence of data, the RSS has been set to zero.

```{r echo=FALSE}
run.this.model <- function(model.num) {
  mw <- 0;
  if (!is.numeric(model.num)) {
    stop("That is not a number, friend. I can't work with something that isn't a number.")
  } else if (model.num < 1) {
    stop("You gave me a model number that doesn't exist, friend. I can't do anything with that.")
  } else if (model.num > 3) {
    stop("You gave me a model number that doesn't exist, friend. I can't do anything with that.")
  } else if (model.num == 1) {
    mw <- model1.weights;
...
while (year.index < end.num) {
  #debug point! print("Hi! I'm year:")
  #debug point! print(year.index)
  #debug point! if(year.index>12) stop() year.index <- 12
  past.group <- get.estimate(mw, past.set, year.index - 1)
  ...
  #debug point! print(now.rsq)
  m.rsq.v <- c(m.rsq.v, now.rsq)
  year.index <- year.index + 1
}
return(m.rsq.v)
}
results <- data.frame("years"=labels.years.ws, row.names = labels.years.ws)
model.nums <- c(1:3)
model.col.names <- "years"
for (n in model.nums) {
  model.str <- paste("Model.Number", as.character(n), sep = ".")
  results$str <- run.this.model(n)
  model.col.names <- c(model.col.names, model.str)
  names(results) <- model.col.names
}
results <- subset(results, select = -years)
kable(results, format = "pipe", caption = "RSS values for Three Weight-differentiated Models")
```

## GRAPHICAL ANALYSIS

After graphing was completed, a problem with the formation (or possibly the premise behind) our models was discovered. As the graph below and the table above show, the RSS values at all years for all the values are functionally identical. We believe that, due to the immense size of these numbers and the storage format of the computer architecture on which they were calculated, the level of detail which would be needed to differentiate these models has been lost.

```{r echo=TRUE, warning=FALSE}
results.ts <- ts(data = results, start = c(1986, 1), end = 2012, frequency = 1, class = "ts", names = c("Model 1", "Model 2", "Model 3"))
plot.ts(x = results.ts, y = NULL, main = "RSS Values of Three Models", xy.labels = TRUE, log = "y", axes = TRUE, type="o")
grid(nx = 23, ny = NA, col = "darkgrey", lty = "dotdash")
```

## CONCLUSION

It is impossible to draw any conclusions, based on this data and analysis, on whether or not this method is or is not usable to estimate future amounts and locations of microplastics in the West Atlantic and Caribbean; this is true with or without changes in weight values for different sets of observations from different years.

### POTENTIAL USES

Despite our inability to draw conclusions about this methodology for modeling datasets like this, further investigation of the code and storage structure of it may be able to do so. The development of our understanding of the current and future behavior of microplastics in our oceans is imperitive. Our current understanding of that microplastics' effects on our own biology [@landrigan2023] and the ecosystems around us [@dusza2022; @dianna.parker2013; @corinaldesi2021; @constant2022; @coralre; @bates2018] demands that we learn more in order to mitigate, and potentially even reverse, its impact.
