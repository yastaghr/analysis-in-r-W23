...
gen.full.wv <- function(model.wv, times, year.index) {
  ...
  print("generating the weights");
    ...
    while (num < end.num) {
        #debug point! 
      print("get weight of year:")
      #debug point! 
      print(num);
        this.w <- model.wv[num];
        #debug point! print("this is that weight");
      #debug point! print(this.w);
      end.of.year <- range.years.ws$End[num];
      start.of.year <- range.years.ws$Start[num];
      ind.start <- length(times[times<=start.of.year]);
      times.v2 <- times[1:ind.start];
      ind.end <- length(times.v2[times.v2<=end.of.year]);
      this.times <- times.v2[1:ind.end];
      ...
      #debug point! print(length(this.times));
        ...
        #debug point! 
      print("Here's the first weight vector");
      #debug point! 
        print(this.wv);
        if (this.wv[1] != FALSE) {
          #debug point! print("if the above was false, this is bad");
          #debug point! 
          print(length(this.wv));
...
get.estimate <- function(model.ws, prev.set, year.index) {
  ...
  print("in get.estimate");
    ...
    #debug point! 
    print("prev.gv - grid values");
    #debug point! 
    print(length(prev.gv$xg));
    #debug point! print(length(prev.gv$yg));
    #debug point! 
    print(length(prev.set$longitude));
