...
overlap.only.v <- function(x1, x2) {
    ...
    new1 <- x1;
    new2 <- x2;
    new.end <- c(length(x1), length(x2));
    if(x1.last>x2.last) {
        new1 <- x1[x1<=x2.last];
        new.end[1] <- length(new1);
    } else if (x1.last<x2.last) {
        new2 <- x2[x2<=x1.last];
        new.end[2] <- length(new2);
    } 
    new.x <- new2;
    new.start <- c(1, 1);
    if(x1.first>x2.first) {
        new.x <- new2[new2>=x1.first];
        new.start[2] <- length(x2[x2<x1.first]);
    } else if (x1.first<x2.first) {
        new.x <- new1[new1>=x2.first];
        new.start[1] <- length(x1[x1<x2.first]);
    }
    return(list("vector" = new.x, "start" = new.start, "stop" = new.end))
}
overlap.v <- function(small.v, big.v) {
    s.1st <- small.v[1];
    b.1st <- big.v[1];
    s.end <- small.v[length(small.v)];
    b.end <- big.v[length(big.v)];
    pad <- 0;
    new.sv <- small.v;
    if (s.1st<b.1st) {
        new.1st <- length(small.v[small.v>=b.1st]);
        new.sv <- new.sv[new.1st:length(new.sv)];
    } else if (s.1st>b.1st) {
        p <- length(big.v[big.v<s.1st]);
        pad <- rep(0, times = p);
        new.sv <- c(pad, new.sv);
    }
    if (s.end>b.end) {
        new.sv <- new.sv[new.sv<=b.end];
    } else if (s.end<b.end) {
        p <- length(big.v[big.v>s.end]);
        pad <- rep(0, times = p);
        new.sv <- c(new.sv, pad);
    }
    return(new.sv)
...
get.diff <- function(est.bi, actual.bi) {
    xover <- overlap.only.v(est.bi$xg, actual.bi$xg);
    shared.x <- xover$vector;
    front.x <- actual.bi$xg[actual.bi$xg<xover$start[2]];
    back.x <- actual.bi$xg[actual.bi$xg>=xover$stop[2]];
    
    yover <- overlap.only.v(est.bi$yg, actual.bi$yg);
    shared.y <- yover$vector;
    front.y <- actual.bi$yg[actual.bi$yg<yover$start[2]];
    back.y <- actual.bi$yg[actual.bi$yg>=yover$stop[2]];
    
    est.ungridded <- ungrid(est.bi$xg, est.bi$yg, est.bi$zg);
    actual.ungridded <- ungrid(actual.bi$xg, actual.bi$yg, actual.bi$zg);
